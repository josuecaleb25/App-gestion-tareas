...
const HabitCard = ({ name, icon, streak, color, dots }) => (
  <div className="bg-[var(--color-card)] rounded-[10px] border border-[var(--color-border)] shadow-sm p-3" style={{ borderLeft: `3px solid ${color}` }}>
    <Icon name={icon} size={18} color={color} className="mb-1.5" />
    <div className="text-[11px] font-semibold text-[var(--color-text)] mb-1">{name}</div>
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--color-background)' }}>
      ...
      <div className="px-5 pt-6 pb-5 rounded-b-3xl mb-2" style={{ background: 'var(--color-card)' }}>
        <h1 className="text-xl font-semibold text-[var(--color-text)]" style={{ fontFamily: 'Fredoka, sans-serif' }}>Seguimiento de Hábitos</h1>
        <p className="text-[11px] text-[var(--color-text-muted)]">5/7 hábitos completados hoy</p>
      </div>
      ...
      <div className="px-4 pb-24">
        ...
        <div className="flex items-center gap-1.5 mt-3 mb-2">
          ...
          <span className="text-sm font-semibold text-[var(--color-text)]" style={{ fontFamily: 'Fredoka, sans-serif' }}>Estado de ánimo hoy</span>
        </div>
        <div className="bg-[var(--color-card)] rounded-2xl border border-[var(--color-border)] shadow-sm p-3.5 mb-3">
          <p className="text-[11px] text-[var(--color-text-muted)] mb-2">¿Cómo te sientes?</p>
        ...
        <div className="flex items-center gap-1.5 mt-4 mb-2">
          ...
          <span className="text-sm font-semibold text-[var(--color-text)]" style={{ fontFamily: 'Fredoka, sans-serif' }}>Mis hábitos</span>
        </div>
        <div className="grid grid-cols-2 gap-2.5 mb-3">
          ...
          {/* Add new habit */}
          <button className="bg-[var(--color-card)] rounded-[10px] border border-[var(--color-border)] border-dashed shadow-sm p-3 opacity-60 flex flex-col items-center justify-center gap-1">
            <Icon name="plus" size={22} color="#8A847C" />
            <span className="text-[11px] font-semibold text-[var(--color-text-muted)]">Nuevo hábito</span>
          </button>
        </div>
        ...
        <div className="flex items-center gap-1.5 mt-4 mb-2">
          ...
          <span className="text-sm font-semibold text-[var(--color-text)]" style={{ fontFamily: 'Fredoka, sans-serif' }}>Nivel de energía</span>
        </div>
        <div className="bg-[var(--color-card)] rounded-2xl border border-[var(--color-border)] shadow-sm p-3.5">
          ...
          <div className="flex justify-between mt-1">
            {energyBars.map((bar, i) => (
              <span key={i} className="text-[9px] text-[var(--color-text-muted)] flex-1 text-center">{bar.day}</span>
            ))}
          </div>
        </div>
      </div>
