  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--color-background)' }}>
      ...
      <div className="px-4 pt-6 pb-3">
        ...
        <h1 className="text-lg font-semibold text-[#5A5550] mb-3" style={{ fontFamily: 'Fredoka, sans-serif' }}>Buscar</h1>

        {/* Search input */}
        <div className="flex items-center gap-2.5 bg-[#FDFAF5] border-[1.5px] rounded-[14px] px-3.5 shadow-md" style={{ borderColor: 'var(--color-primary-light)' }}>
          <Icon name="search" size={16} color="#8A847C" />
          <input 
            type="text"
            placeholder="Tareas, notas, hábitos..."
            className="flex-1 py-3 text-sm text-[#5A5550] bg-transparent outline-none placeholder:text-[#8A847C]"
          ...
          {categories.map((cat, i) => (
            <button 
              ...
              className={`px-3.5 py-1.5 rounded-full text-[11px] font-semibold border-[1.5px] whitespace-nowrap ${
                cat.active 
                  ? 'text-white' 
                  : 'bg-[#FDFAF5] border-[rgba(168,137,108,0.15)] text-[#8A847C]'
              }`}
      ...
      <div className="px-4 pb-24">
        ...
        <div className="flex items-center gap-1.5 mt-3 mb-2">
          ...
          <span className="text-sm font-semibold text-[#5A5550]" style={{ fontFamily: 'Fredoka, sans-serif' }}>Recientes</span>
        </div>

        {notes.map((note, i) => (
          <div key={i} className="bg-[#FDFAF5] rounded-2xl border border-[rgba(168,137,108,0.15)] shadow-sm p-3.5 mb-2.5 relative overflow-hidden">
            <div className="absolute top-0 left-0 bottom-0 w-1 rounded-l-2xl" style={{ background: note.color }}></div>
            <div className="pl-2.5">
              <div className="text-sm font-semibold text-[#5A5550]" style={{ fontFamily: 'Fredoka, sans-serif' }}>{note.title}</div>
              <div className="text-[11px] text-[#8A847C] leading-relaxed mt-1">{note.preview}</div>
              <div className="text-[10px] text-[#8A847C] font-semibold mt-2">{note.date}</div>
            </div>
          </div>
        ))}
        ...
        <div className="flex items-center gap-1.5 mt-5 mb-2">
          ...
          <span className="text-sm font-semibold text-[#5A5550]" style={{ fontFamily: 'Fredoka, sans-serif' }}>Explorar categorías</span>
        </div>
        <div className="grid grid-cols-2 gap-2.5">
          {exploreCategories.map((cat, i) => (
            <button key={i} className="bg-[#FDFAF5] rounded-[10px] border border-[rgba(168,137,108,0.15)] shadow-sm p-2.5 flex items-center gap-2 hover:bg-[#F5F0E8] transition-colors">
              <Icon name={cat.icon} size={18} color="#5A5550" />
              <span className="text-xs font-semibold text-[#5A5550]">{cat.label}</span>
            </button>
          ))}
        </div>
      </div>
